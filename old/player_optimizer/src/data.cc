...
vector<long double> Data::getAllScores ()
{
...
vector<long double> Data::getAllOwnerships ()
{
    int num_players = this->players_.size ();
    vector<long double> ownerships;
    for (int i = 0; i < num_players; ++i)
    {
        ownerships.push_back (this->players_[i].getProjection()->getOwnerships());
    }
    return ownerships;
}

map< string, vector<long int> > Data::getTeamPlayers ()
{
    int num_players = this->players_.size ();
    map< string, vector<long int> > team_players;
    for (int i = 0; i < num_players; ++i)
    {
        ...
        {
            team_players[team] = vector<long int> {this->players_[i].getId()};
        }
...
map< string, vector<long int> > Data::getPositionPlayers ()
{
    int num_players = this->players_.size ();
    map< string, vector<long int> > position_players;
    for (int i = 0; i < num_players; ++i)
    {
        ...
        {
            position_players[position] = vector<long int> {this->players_[i].getId()};
        }
...
map< string, vector<long int> > Data::getTeamDefenders ()
{
    int num_players = this->players_.size ();
    map< string, vector<long int> > team_players;
    for (int i = 0; i < num_players; ++i)
